classdef FreqrespMode
    methods (Static)
        function activate(app)
            ...
            modes.FreqrespMode.sizing(app);
            modes.FreqrespMode.resize(app);
        ...
        function sizing(app)
            ...
            app.axes(1).YLabel.String = 'Voltage (V)';
            app.axes(1).YLabel.Color = 'k';
            app.axes(1).XLabel.String = 'Time (s)';
            app.axes(1).XLabel.Color = 'k';
            app.axes(1).YLim = [-0.2 0.2];
            app.axes(1).XLim = [0 0.005];

            app.axes(2) = axes('Parent',app.panel,'Position',[0.55 0.05 0.45 0.90],...
                'GridLineStyle','-','XColor', 0.15*[1 1 1],'YColor', 0.15*[1 1 1]);
            set(app.axes(2),'NextPlot','add','XLimMode','manual');
            set(app.axes(2),'XGrid','on','YGrid','on','Tag','Axes','Box','on','XScale','log');
            app.axes(2).YLabel.String = 'Gain (dB)';
            app.axes(2).YLabel.Color = 'k';
            app.axes(2).XLabel.String = 'Frequency (Hz)';
            app.axes(2).XLabel.Color = 'k';
            app.axes(2).YLim = [-120 60];
            app.axes(2).XLim = [1 5e4];
            ...
            function reset_fig
                app.axes(1).YLim = [-0.2 0.2];
                app.axes(2).YLim = [-120 60];
                app.axes(1).XLim = [0 0.1];
                app.axes(2).YLim = [1 5e4];
            end
            ...
            app.tbuts(1) = uicontrol('Parent', app.panel, ...
                ...
                'callback', @(src,~) app.stateDecision(src), 'tag', 'freqresp_record');
            app.tbuts(2) = uicontrol('Parent', app.panel, ...
                'Style', 'togglebutton', 'CData', imread('Play.png'),...
                'callback', @(src,~) app.stateDecision(src), 'tag', 'freqresp_play');
            % set the resize function
            set(app.panel, 'ResizeFcn', @(~,~) app.freqrespResizeFcn);
