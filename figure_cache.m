classdef figure_cache < handle
    ...
    properties
        nsigs = 4; % number of channels being displayed
        xmax = 5; % maximum limit of x axis (time in seconds)
        ymax = 1; % maximum (symmetric) limit of y axis
        alpha = ones(1,4); % scaling factor to turn input voltage (-10 to 10 V) into signal
        ...
        channelVisible = true(1,8); % visibility flag for each channel
    end
    ...
    methods
        ...
        function clear_fig(obj)
            ...
            xlim(obj.ax, [0 obj.xmax]);
            xlabel(obj.ax, 'Time (s)');
            ylabel(obj.ax, 'Current (pA)')
            grid(obj.ax,'on');
        end
        ...
        function draw_fig_now(obj)
            ...
            try
                ...
                if isempty(u)
                    ...
                    for i = 1:obj.nsigs
                        if i <= numel(u)
                            if obj.channelVisible(i)
                                if obj.sparse
                                    set(u(obj.nsigs - i + 1),'YData',obj.ydata(:,i), ...
                                        'Color',obj.cmap(i,:), ...
                                        'LineStyle','none','Marker','.', 'Visible', 'on');
                                else
                                    set(u(obj.nsigs - i + 1),'YData',obj.ydata(:,i), ...
                                        'Color',obj.cmap(i,:), ...
                                        'LineStyle','-','Marker','none', 'Visible', 'on');
                                end
                            else
                                % Hide the channel
                                set(u(obj.nsigs - i + 1), 'Visible', 'off');
                            end
                        end
                    end
                end
    ...
    methods (Access = private)
        ...
        function d = downsample_minmax(obj, data)
            ...
            obj.sparse = false;
            data = filt_lpb(data,4,1000);%1000Hz
            data = data(51:end-50,:); %????
            ...
            if 2*dspoints >= size(data,1)
                ...
                logic = logical(logic(:,randperm(size(logic,2)))); % random rearrangement
                [~,inds] = find(logic==1);
        ...
        function d = downsample_random(obj, data)
            ...
            if dspoints >= size(data,1)
                ...
                logic = [ones(1,dspoints), zeros(1,size(data,1)-dspoints)]; % dspoints number of ones
                logic = logical(logic(:,randperm(size(logic,2))));%boolean(logic(:,randperm(size(logic,2)))); % random rearrangement
                d = data(logic',:);
            end
        end
